...
public static class MlKem768
{
    ...
    internal static (byte[] ek, byte[] dk) KeyGenInternal(byte[] d, byte[] z)
    {
    ...
    internal static (byte[] sharedSecret, byte[] ciphertext) EncapsInternal(byte[] ek, byte[] m)
    {
        // (K, r) = G(m || H(ek))
        byte[] hEk = HashH(ek);
    ...
    private static void ValidateEncapsulationKey(byte[] ek)
    {
        ...
        for (int i = 0; i < K; i++)
        {
            ...
            for (int j = 0; j < N; j++)
                if (poly[j] >= Q)
                    throw new TlsException(AlertDescription.IllegalParameter,
                        "ML-KEM-768 encapsulation key has an out-of-range coefficient (FIPS 203 §7.2 modulus check)");
            offset += PolyBytes;
        }
    }
