...
{
    // TODO Do these curves have names; can we make this class look more like a "normal" NamedCurves registry?
    public class Dstu4145NamedCurves
    {
        ...
        private static readonly ECDomainParameters[] DomainParameters = new ECDomainParameters[CurveCount];
        private static readonly DerObjectIdentifier[] Oids = new DerObjectIdentifier[CurveCount];

        //All named curves have the following oid format: 1.2.804.2.1.1.1.1.3.1.1.2.X
        //where X is the curve number 0-9
        private static readonly DerObjectIdentifier OidBase = UAObjectIdentifiers.dstu4145le.Branch("2");
        ...
        static Dstu4145NamedCurves()
        {
            BigInteger[] n_s = new BigInteger[CurveCount];
            ...
            BigInteger[] h_s = new BigInteger[CurveCount];
            h_s[0] = Two;
            h_s[1] = Two;
            h_s[2] = Four;
            h_s[3] = Two;
            ...
            F2mCurve[] curves = new F2mCurve[CurveCount];
            curves[0] = new F2mCurve(163, 3, 6, 7, One, new BigInteger("5FF6108462A2DC8210AB403925E638A19C1455D21", 16), n_s[0], h_s[0]);
            curves[1] = new F2mCurve(167, 6, One, new BigInteger("6EE3CEEB230811759F20518A0930F1A4315A827DAC", 16), n_s[1], h_s[1]);
            curves[2] = new F2mCurve(173, 1, 2, 10, Zero, new BigInteger("108576C80499DB2FC16EDDF6853BBB278F6B6FB437D9", 16), n_s[2], h_s[2]);
            curves[3] = new F2mCurve(179, 1, 2, 4, One, new BigInteger("4A6E0856526436F2F88DD07A341E32D04184572BEB710", 16), n_s[3], h_s[3]);
            curves[4] = new F2mCurve(191, 9, One, new BigInteger("7BC86E2102902EC4D5890E8B6B4981ff27E0482750FEFC03", 16), n_s[4], h_s[4]);
            curves[5] = new F2mCurve(233, 1, 4, 9, One, new BigInteger("06973B15095675534C7CF7E64A21BD54EF5DD3B8A0326AA936ECE454D2C", 16), n_s[5], h_s[5]);
            curves[6] = new F2mCurve(257, 12, Zero, new BigInteger("1CEF494720115657E18F938D7A7942394FF9425C1458C57861F9EEA6ADBE3BE10", 16), n_s[6], h_s[6]);
            curves[7] = new F2mCurve(307, 2, 4, 8, One, new BigInteger("393C7F7D53666B5054B5E6C6D3DE94F4296C0C599E2E2E241050DF18B6090BDC90186904968BB", 16), n_s[7], h_s[7]);
            curves[8] = new F2mCurve(367, 21, One, new BigInteger("43FC8AD242B0B7A6F3D1627AD5654447556B47BF6AA4A64B0C2AFE42CADAB8F93D92394C79A79755437B56995136", 16), n_s[8], h_s[8]);
            curves[9] = new F2mCurve(431, 1, 3, 5, One, new BigInteger("03CE10490F6A708FC26DFE8C3D27C4F94E690134D5BFF988D8D28AAEAEDE975936C66BAC536B18AE2DC312CA493117DAA469C640CAF3", 16), n_s[9], h_s[9]);

            ECPoint[] points = new ECPoint[CurveCount];
            ...
            {
                DomainParameters[i] = new ECDomainParameters(curves[i], points[i], n_s[i], h_s[i]);
                Oids[i] = OidBase.Branch("" + i);
            }
        }
         ...
         * where X is the curve number 0-9
         */
        public static DerObjectIdentifier[] GetOids() => Arrays.CopyBuffer(Oids);
