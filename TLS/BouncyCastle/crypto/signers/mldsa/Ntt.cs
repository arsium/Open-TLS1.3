...
﻿namespace Org.BouncyCastle.Crypto.Signers.MLDsa
{
    internal static class Ntt
    {
        ...
        public static void NTT(int[] r)
        {
            ...
            {
                for (int start = 0; start < MLDsaEngine.N; start = j + len)
                {
                    int zeta = Zetas[++k];
                    for (j = start; j < start + len; ++j)
                    {
                        int r0 = r[j];
                        int t = Reduce.MontgomeryReduce((long)zeta * r[j + len]);
                        r[j] = r0 + t;
                        r[j + len] = r0 - t;
                    }
                }
        ...
        public static void InverseNttToMont(int[] a)
        {
            ...
            for (int len = 1; len < n; len <<= 1)
            {
                for (int start = 0; start < n; start = j + len)
                {
                    ...
                    for (j = start; j < start + len; ++j)
                    {
                        int a0 = a[j], a1 = a[j + len];
                        a[j] = a0 + a1;
                        a[j + len] = Reduce.MontgomeryReduce((long)zeta * (a0 - a1));
                    }
                }
            }
            ...
            for (j = 0; j < n; ++j)
            {
                a[j] = Reduce.MontgomeryReduce((long)f * a[j]);
            }
        }
    }
}
