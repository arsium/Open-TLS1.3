...
public abstract class CMAC : KeyedHashAlgorithm
{
    ...
    public string SymmetricAlgorithmName
    {
        ...
        set
        {
            ...
            // Resolve GOST ciphers directly (AOT/trim-safe; avoids the obsolete CryptoConfig name lookup).
            _symmetricAlgorithm = value switch
            {
                CryptoConstants.GrasshopperAlgorithmName => new GrasshopperManaged(),
                CryptoConstants.MagmaAlgorithmName => new MagmaManaged(),
                _ => throw new CryptographicException(CryptographyStrings.CryptographicUnknownSymmetricAlgorithm, value),
            };
